...
# Load package list from packages-list.gz if available, else use defaults
load_packages() {
    if [ -f "$SCRIPT_DIR/packages-list.gz" ]; then
        zcat "$SCRIPT_DIR/packages-list.gz" | awk '
            BEGIN { RS=""; FS="\n" }
            { pkg=""; file="" }
            { for(i=1;i<=NF;i++) {
                if($i ~ /^Package: /) pkg=substr($i,10)
                if($i ~ /^Filename: /) file=substr($i,10)
            }
            if(pkg && file && file ~ /aarch64\.deb$/) print pkg"\t"file }
        '
    fi
}
...
echo "=== EZOS Package Builder ==="
echo ""

# Try auto-detect from packages-list.gz
if [ -f "$SCRIPT_DIR/packages-list.gz" ]; then
    echo "Auto-detecting packages from packages-list.gz..."
    count=0
    load_packages > "$DEB_DIR/_pkglist.txt"
    while IFS=$'\t' read -r pkg deb_path; do
        [ -z "$pkg" ] && continue
        build_one "$pkg" "$deb_path" || echo "FAILED: $pkg"
        count=$((count+1))
    done < "$DEB_DIR/_pkglist.txt"
    rm -f "$DEB_DIR/_pkglist.txt"
    echo ""
    echo "Built $count packages."
else
    echo "No packages-list.gz found. Building hardcoded list..."
